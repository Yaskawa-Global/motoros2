...
BOOL Ros_Controller_Initialize()
{
    ...
    const BOOL bNeedToWarnAboutCalib = Ros_Controller_ShouldWarnNoCalibDataLoaded(
        &g_Ros_Controller, g_nodeConfigSettings.publish_tf, bCalibLoadedOk);
    ...
    if (bNeedToWarnAboutCalib)
    {
        ...
        {
            mpSetAlarm(ALARM_CONFIGURATION_FAIL, "No calibration: check TF",
                SUBCODE_CONFIGURATION_NO_CALIB_FILES_LOADED);
            Ros_Debug_BroadcastMsg(
                "%s: no calibration files loaded: TF potentially incorrect -> warn "
...
BOOL Ros_Controller_ShouldWarnNoCalibDataLoaded(Controller const* controller, BOOL bCalibLoadedOk, BOOL bPublishTfEnabled)
{
    ...
    return (
        //single-group systems don't have calibration files at all
        controller->numGroup > 1 &&
        //only Robots can be calibrated to other groups, so we need at least one Robot
