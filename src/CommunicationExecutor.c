...
void Ros_Communication_ConnectToAgent()
{
    ...
    memcpy(&client_key, g_Ros_Controller.rosTrafficMacAddr + 2, sizeof(client_key));
...
void Ros_Communication_StartExecutors(SEM_ID semCommunicationExecutorStatus)
{
    ...
    rc = rclc_executor_add_service(
        &executor_motion_control, &g_serviceQueueTrajPointStream, g_messages_QueueTrajPointStream.request,
        g_messages_QueueTrajPointStream.response, Ros_ServiceQueueTrajPointStream_Trigger);
