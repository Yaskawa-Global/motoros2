...
*Solution:*
This alarm should always be accompanied by another alarm that specifies where the alarm occurred. This alarm occurs because
some call to the rcl or rclc API failed. The return value of type `rcl_ret_t` for some rcl or rclc API call was expected to
be `RCL_RET_OK`, but the return value was instead indicative of some error. The actual rcl return value is given in the text
